...
def create_serialization_dir(
    ...
    if recover and force:
        raise ConfigurationError(
            "Illegal arguments: both force and recover are true.")
...
def evaluate(model: Model,
    ...
    with torch.no_grad():
        ...
        generator_tqdm = Tqdm.tqdm(data_iterator)
        ...
        for batch in generator_tqdm:
            ...
            if loss is not None:
                ...
                predicted_labels = torch.argmax(
                    output_dict['logits'], dim=1).cpu().numpy()
                ground_truth = batch['label'].squeeze(-1).cpu().numpy()
                incorrect_indices = (predicted_labels !=
                                     ground_truth).nonzero()
                incorrect_indices = incorrect_indices[0]
                if len(incorrect_indices) > 0:
                    # Convert indices to absolute indices in the dataset
                    print(incorrect_indices)
                    misclassified_user_ids = [
                        batch['user_id'][idx] for idx in incorrect_indices]
                    print("Misclassified:", misclassified_user_ids)
                    misclassified_instances.extend(misclassified_user_ids)
        ...
        final_metrics["misclassified_instances"] = misclassified_instances
        print("Misclassified Instances:",
              final_metrics['misclassified_instances'])
...
def statistics_rumour_dnn_dataset(file_name):
    ...
    social_context_data_dir = os.path.join(os.path.dirname(
        __file__),  '..', "data", "social_context", "aug-rnr-annotated-threads-retweets")
    ...
    std_reactions = 0 if len(
        all_replies_list) == 0 else statistics.stdev(all_replies_list)
    ...
    median_reactions = 0 if len(
        all_replies_list) == 0 else statistics.median(all_replies_list)
    ...
    std_retweets = 0 if len(
        all_retweets_list) == 0 else statistics.stdev(all_retweets_list)
    # Median has a very big advantage over Mean, which is the median value is not skewed so much by extremely large or small values,
    # see also https://www.geeksforgeeks.org/python-statistics-median/
    median_retweets = 0 if len(
        all_retweets_list) == 0 else statistics.median(all_retweets_list)
...
def count_social_context(source_tweet_id, context_tweets_dataset_dir_dict):
    ...
    for c_type in context_types:
        all_context_data_dir = os.path.join(
            context_tweets_dataset_dir, "{}".format(c_type))
...
def shuffle_dataset(csv_file_path: str):
    ...
    ds.to_csv(csv_file_path.replace(".csv", "_shuffled.csv"),
              encoding="utf-8", quoting=csv.QUOTE_MINIMAL, index=False)


def test_dataset(csv_file_path: str):
    df = load_matrix_from_csv(
        csv_file_path, start_col_index=0, end_col_index=4, header=0)
    rumor_num = 0
    non_rumor_num = 0
    for tweet_row in df[:]:
...
def generate_shuffled_training_set():
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "bostonbombings", "aug_rnr_train_set_combined.csv"))
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "bostonbombings", "aug_rnr_heldout_set_combined.csv"))

    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "charliehebdo", "aug_rnr_train_set_combined.csv"))
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "charliehebdo", "aug_rnr_heldout_set_combined.csv"))

    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..',
                    "data", "train", "ferguson", "aug_rnr_train_set_combined.csv"))
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..',
                    "data", "train", "ferguson", "aug_rnr_heldout_set_combined.csv"))

    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..',
                    "data", "train", "germanwings", "aug_rnr_train_set_combined.csv"))
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "germanwings", "aug_rnr_heldout_set_combined.csv"))

    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "ottawashooting", "aug_rnr_train_set_combined.csv"))
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "ottawashooting", "aug_rnr_heldout_set_combined.csv"))

    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..',
                    "data", "train", "sydneysiege", "aug_rnr_train_set_combined.csv"))
    shuffle_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                    "train", "sydneysiege", "aug_rnr_heldout_set_combined.csv"))

    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "bostonbombings", "aug_rnr_train_set_combined_shuffled.csv"))
    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data", "train",
                 "bostonbombings", "aug_rnr_heldout_set_combined_shuffled.csv"))

    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "charliehebdo", "aug_rnr_train_set_combined_shuffled.csv"))
    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "charliehebdo", "aug_rnr_heldout_set_combined_shuffled.csv"))

    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "ferguson", "aug_rnr_train_set_combined_shuffled.csv"))
    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "ferguson", "aug_rnr_heldout_set_combined_shuffled.csv"))

    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "germanwings", "aug_rnr_train_set_combined_shuffled.csv"))
    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "germanwings", "aug_rnr_heldout_set_combined_shuffled.csv"))

    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "ottawashooting", "aug_rnr_train_set_combined_shuffled.csv"))
    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data", "train",
                 "ottawashooting", "aug_rnr_heldout_set_combined_shuffled.csv"))

    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "sydneysiege", "aug_rnr_train_set_combined_shuffled.csv"))
    test_dataset(os.path.join(os.path.dirname(__file__),  '..', "data",
                 "train", "sydneysiege", "aug_rnr_heldout_set_combined_shuffled.csv"))
